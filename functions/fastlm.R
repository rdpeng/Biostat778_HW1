fastlm=function(X,y,na.rm=FALSE){ # using cholesky decomposition.
  size = dim(X)
  n = size[1]
  p = size[2]
...
#  beta=solve(QR,y)
#  resid=qr.resid(QR,y)
#  var_Y=sum(resid^2)/(n-p)
#  FAST=list(coef=bete,residual=resid,sigma.2=var_Y)
#  FAST
#}
