...
function(x, mu, S, log = TRUE) {
  U = tryCatch(chol(S),
      error = function(cond){stop("S is not positive definite")})  
                                           # by using cholesky decomposition, the positive-definity is checked.
  ...
  k = dim(x)[2]
    ...
    D2 = crossprod(D1)[1,1]                # compute the mahalanobis distance
